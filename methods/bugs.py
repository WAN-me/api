...
def get(args):
    ...
    if ss == True: 
        ...
        if not thisuser or len(thisuser)!=1:
            ...
            thisuser = thisuser[0]
            bug = (db.exec('''select id,title,priority,case,actual,expected,user_id,status,product from bugs where id = :id ''',{'id':id}))
            product = bug['product']
            if len(bug) == 0:
            ...
            if(product['type']<1):
                if not(thisuser[0] in product['users'] or thisuser[0] in product['admins'] or thisuser[0] == product['owner_id']):
                    return utils.error(403,"you are not tester for this product")
                        ...
                        return {'id':bug[0],'title':bug[1],'priority':bug[2],'case':bug[3],'actual':bug[4],'expected':bug[5],"user_id":bug[6],"status":bug[7],"product":bug[8]}
    else:
        return ss

def new(args):
    ss = utils.notempty(args,['accesstoken','title','priority','case','actual','expected','product'])
    if ss == True: 
        token = args['accesstoken']
        ...
        if not thisuser or len(thisuser)!=1:
            ...
            if(product['type']<1):
                if not(thisuser[0] in product['users'] or thisuser[0] in product['admins'] or thisuser[0] == product['owner_id']):
                    ...
                    db.exec('''insert into bugs (title,priority,case,actual,expected,user_id,product)
                    values (?,?,?,?,?,?,?)''',(args['title'],args['priority'],args['case'],args['actual'],args['expected'],thisuser[0],args['product'],))
