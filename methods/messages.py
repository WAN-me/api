...
def send(args):
    ...
    if ss == True: 
        ...
        pool._set(1,to_id,msg_id,{'id':msg_id,'from_id':user[0],'to_id':to_id,'text':secure(text)}) # добавляем события
        pool._set(2,user[0],msg_id,{'id':msg_id,'from_id':user[0],'to_id':to_id,'text':secure(text)})
        chats._set(user[0],to_id,users._get(to_id)['name']) # Добавляем в список чатов
...
def gethistory(args):
    ...
    if ss == True: 
        ...
        for raw_message in raw_messages:
            messages.append({'id':raw_message[0],'from_id':raw_message[1],'to_id':raw_message[2],'text':secure(raw_message[3]),'time':raw_message[4]})
