...
def changepass(args):
    ...
    if ss == True: 
        ...
        result = db.exec(f'''select from users where password={oldpass} and token={token}''')
        if len(result)==0:
            return utils.error(401,"password is incorrect")
        db.exec(f'''update users set token={token},password={newpass} where password={oldpass} and token={token}''')
        return {"state":'ok'}
    else:
        return ss
...
def addsocial(args):
    ...
    if ss == True: 
        ...
        if social_name == 'vk':
            response = json.loads(requests.get(f"https://api.vk.com/method/users.get?access_token={social_token}&v=5.101").content)
